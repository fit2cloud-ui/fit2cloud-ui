<template>
  <el-table-column class-name="fu-table-operations" :align="align" :width="computeWidth" v-bind="$attrs" v-on="$listeners"
                   v-if="showButtons">
    <template #header>
      {{ $attrs.label }}
      <fu-table-column-select class="fu-" type="dialog" :columns="columns"/>
    </template>
